      ...
      <el-table v-loading="listLoading" :data="dataList" :max-height="clientHeight" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
        ...
        <el-table-column align="right" label="操作" width="155" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="authCheck(45) && scope.row.id" type="info" size="mini" plain icon="el-icon-document" @click="detail(scope.row.id,scope.row.sid)">详情</el-button>
            <el-button v-if="authCheck(46) && scope.row.id" type="primary" size="mini" plain icon="el-icon-edit" @click="edit(scope.row.id,scope.row.sid)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { cxnlList } from "@/api/grxx/nlcp/cxnl";
...
export default {
  ...
  data() {
    return {
      ...
      listLoading: false,
      total: 0,
      dataList: [{ id: 1, name: 'xxxx', account: '11100', ljsw: 8, bzsw: 8.5, cxys: 9, yjpz: 7 }],
      // 检索参数
      listQuery: {
