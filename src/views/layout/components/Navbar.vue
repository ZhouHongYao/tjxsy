...
import { yxList } from "@/api/grxx/jbxx";
import { cateList } from "@/api/system/category";
...
export default {
  ...
  methods: {
    ...
    setHc() {
      cateList()
        .then(rs => {
          let zzmm = rs.find(v => { return v.id == 7 })
          if (zzmm && zzmm.children) {
            this.$store.dispatch('setZzmm', zzmm.children)
          }
          let hjdj = rs.find(v => { return v.id == 15 })
          if (hjdj && hjdj.children) {
            this.$store.dispatch('setHjdj', hjdj.children)
          }
          let xmlb = rs.find(v => { return v.id == 22 })
          if (xmlb && xmlb.children) {
            this.$store.dispatch('setXmlb', xmlb.children)
          }
        })
        .catch(() => { });
    },
    ...
    getYx() {
      yxList().then(rs => {
        this.$store.dispatch('setXyData', rs)
      })
        .catch(() => { });
    }
  }
