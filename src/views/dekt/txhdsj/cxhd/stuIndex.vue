<template>
  <div>
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="活动报名" name="first">
          ...
          <el-table v-loading="listLoading" :data="dataList" :max-height="clientHeight" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
            ...
            <el-table-column min-width="160" prop="bmjz_time" label="活动截止报名时间" sortable="custom" />
            <el-table-column min-width="270" prop="times" label="活动时间">
            ...
            <el-table-column min-width="150" prop="address" label="活动地点" show-overflow-tooltip />
            <el-table-column :width="90" align="center" label="操作" fixed="right" class-name="small-padding fixed-width">
              <template slot-scope="d">
                <span v-if="checkTime(d.row.times[1])" class="text-muted">活动已结束</span>
                <span v-else>
        ...
        <el-tab-pane label="我的活动" name="second">
          <div class="filter-container">
            <el-input :clearable="true" v-model="listQuery.keys" style="width: 200px;" class="filter-item" placeholder="检索：活动名称" @keyup.enter.native="handleFilter" />
            <el-button v-waves class="filter-item margin-right-10" type="primary" plain icon="el-icon-search" @click="handleFilter">搜索</el-button>
          </div>
          <el-table v-loading="listLoading" :data="dataList" :max-height="clientHeight" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
            ...
            <el-table-column :width="200" align="right" label="操作" fixed="right" class-name="small-padding fixed-width">
              <template slot-scope="d">
                <span v-if="isTj(d.row.times[1])" class="text-warning margin-right-10">活动尚未结束</span>
                <span v-else>
                  <el-button type="primary" size="mini" plain icon="el-icon-edit" @click="edit(d.row.id)">反馈</el-button>
                </span>
    ...
    <NewEdit :dialog-state="dialogState" :hdid="hdid" @CB-hd="CB_hd" />
  </div>
</template>

<script>
import waves from "@/directive/waves"; // 水波纹指令
import NewEdit from "../component/newEdit";
