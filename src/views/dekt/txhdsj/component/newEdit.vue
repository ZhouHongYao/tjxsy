<template>
  <div>
    <el-dialog v-el-drag-dialog :visible="dialogState" :close-on-click-modal="false" :title="form.title" :fullscreen="isphone()" width="780px" top="5vh" @close="handleClose" @open="openDialog">
      <div :style="{'height':isphone() ? 'auto' : clientHeight +'px', 'overflow-y': 'auto'}">
        <el-form v-loading="listloading" ref="form" :model="form" :rules="rules" class="padding-right-5" label-suffix="：" label-width="120px" element-loading-text="数据载入中">
          ...
          <el-form-item label="具体参与情况" prop="cyqk">
            <el-input :autosize="{ minRows: 5, maxRows: 8}" v-model="form.cyqk" type="textarea" maxlength="500" placeholder="具体承担的工作任务" />
            还可填写
            <span class="text-danger">{{ 500-form.cyqk.length }}</span> 个字符
          </el-form-item>
          <el-form-item label="活动收获与感悟" prop="shgw" label-width="125px">
            <el-input :autosize="{ minRows: 5, maxRows: 8}" v-model="form.shgw" type="textarea" maxlength="500" placeholder="活动收获与感悟" />
            还可填写
            <span class="text-danger">{{ 500-form.shgw.length }}</span> 个字符
          </el-form-item>
          <el-form-item label="活动反馈" prop="hdfk">
            <el-input :autosize="{ minRows: 5, maxRows: 8}" v-model="form.hdfk" type="textarea" maxlength="500" placeholder="活动反馈" />
            还可填写
