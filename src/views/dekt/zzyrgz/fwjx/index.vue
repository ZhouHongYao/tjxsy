  ...
  <div>
    <div class="app-container">
      ...
      <el-table v-loading="listLoading" :data="dataList" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
        ...
        <el-table-column min-width="300" prop="jtzr" label="在承担家庭责任方面做了哪些努力" show-overflow-tooltip />
        <el-table-column min-width="300" prop="jxgz" label="对家乡的发展有哪些关注" show-overflow-tooltip />
        <el-table-column min-width="300" prop="fwjx" label="在服务家乡发展方面做了哪些工作" show-overflow-tooltip />
        <el-table-column align="right" label="操作" width="80" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="authCheck(156) && scope.row.id" type="info" size="mini" plain icon="el-icon-document" @click="detail(scope.row.id,scope.row.sid)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { fwjxList } from "@/api/dekt/zzyrgz/fwjx";
