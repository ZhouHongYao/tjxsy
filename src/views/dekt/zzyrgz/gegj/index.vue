  ...
  <div>
    <div class="app-container">
      ...
      <el-table v-loading="listLoading" :data="dataList" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
        ...
        <el-table-column min-width="400" prop="content" label="接受资助后的感想（对老师、学校、社会、国家层面的）" show-overflow-tooltip />
        <el-table-column align="right" label="操作" width="80" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="authCheck(160) && scope.row.id" type="info" size="mini" plain icon="el-icon-document" @click="detail(scope.row.id,scope.row.sid)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { gegjList } from "@/api/dekt/zzyrgz/gegj";
