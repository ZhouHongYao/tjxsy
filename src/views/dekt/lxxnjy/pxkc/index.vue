  ...
  <div>
    <div class="app-container">
      <div class="filter-container">
        ...
        <el-button v-waves v-if="authCheck(234)" class="filter-item margin-right-10" type="primary" icon="el-icon-circle-plus-outline" @click="add">添加</el-button>
      ...
      <el-table v-loading="listLoading" :data="dataList" :max-height="clientHeight" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort" @selection-change="handleSelectionChange">
        ...
        <el-table-column min-width="130" prop="kcbh" label="课程编号" show-overflow-tooltip sortable="custom" />
        ...
        <el-table-column min-width="130" prop="fbzt" label="发布状态" show-overflow-tooltip sortable="custom">
          <template slot-scope="d">
            {{ d.row.fbzt ? d.row.fbzt== 2 ? '是' : '否' : '--' }}
          </template>
        </el-table-column>
        <el-table-column min-width="200" prop="beizhu" label="备注" show-overflow-tooltip />
        <el-table-column align="right" label="操作" width="160" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="authCheck(236)" type="primary" size="mini" plain icon="el-icon-edit" @click="edit(scope.row.id)">编辑</el-button>
            <el-button v-if="authCheck(237)" type="danger" size="mini" plain icon="el-icon-delete" @click="deleteData(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
import { pxkcList, pxkcDel } from "@/api/dekt/lxxnjy/pxkc";
