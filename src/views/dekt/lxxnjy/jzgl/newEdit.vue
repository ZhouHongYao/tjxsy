  ...
  <el-dialog v-el-drag-dialog :title="title" :visible="dialogState" :fullscreen="isphone()" :close-on-click-modal="false" width="700px" top="10vh" @close="handleClose" @open="openDialog">
    <div :style="{'height':isphone() ? 'auto' : clientHeight +'px', 'overflow-y': 'auto'}">
      <el-form v-loading="listloading" ref="form" :model="form" :rules="rules" class="padding-right-5" label-suffix="：" label-width="120px" element-loading-text="数据载入中">
        <el-form-item :label="mc+'名称'" prop="title">
          <el-input v-model="form.title" placeholder="请填写名称" clearable />
        </el-form-item>
        <el-form-item label="报名截止时间" prop="bmjz_time">
          <el-date-picker v-model="form.bmjz_time" class="filter-item" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" type="datetime" placeholder="报名截止时间" align="right" />
        </el-form-item>
        <el-form-item :label="mc+'时间'" prop="times">
          <el-date-picker :unlink-panels="true" v-model="form.times" class="filter-item" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" type="datetimerange" start-placeholder="开始时间" end-placeholder="结束时间" align="right" />
        </el-form-item>
        <el-form-item :label="mc+'人数'" prop="num">
          <el-input v-model="form.num" placeholder="请填写人数" clearable />
        </el-form-item>
        <el-form-item :label="mc+'地点'" prop="address">
          <el-input v-model="form.address" placeholder="请填写地点" clearable />
        </el-form-item>
    ...
    <el-dialog :visible="memberStatus" :close-on-click-modal="false" :close-on-press-escape="false" title="添加组织者" width="500px" top="30vh" append-to-body @close="close" @open="open">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-waves :loading="loading" :disabled="item ? false:'' " icon="el-icon-success" type="primary" @click="createData"> 保存</el-button>
      </div>
    </el-dialog>
  </el-dialog>
</template>
...
export default {
  ...
  props: ["dialogState", "hdId", "type", "mc"],
  data() {
    return {
      title: "",
      listloading: false,
  ...
  methods: {
    openDialog() {
      ...
      if (this.hdId) {
        this.title = "编辑" + this.mc;
        this.getRead();
      } else {
        this.title = "添加" + this.mc;
      }
    },
