      ...
      <div class="filter-container">
        ...
        <span v-if="currentRole()">
          <span v-if="info.can_time && info.can_time.is_can">
            <el-button v-waves v-if="authCheck(275)" class="filter-item margin-right-10" type="primary" icon="el-icon-circle-plus-outline" @click="add()">添加</el-button>
          </span>
      ...
      <el-table v-loading="listLoading" :data="dataList" element-loading-text="数据载入中" highlight-current-row fit stripe @sort-change="handleSort">
        ...
        <el-table-column align="right" label="操作" width="155" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-if="authCheck(276)" type="info" size="mini" plain icon="el-icon-document" @click="detail(scope.row.id,scope.row.sid)">详情</el-button>
            <el-dropdown v-if="authCheck(277) || authCheck(278)" class="margin-left-10" trigger="click" @command="handleCommand">
              <el-button type="info" size="mini" plain>
                更多
                <i class="el-icon-arrow-down el-icon--right" />
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="authCheck(277)" :command="{type:'edit',ID:scope.row.id,SID:scope.row.sid}">
                  <span class="text-primary">编辑</span>
                </el-dropdown-item>
                <el-dropdown-item v-if="authCheck(278)" :command="{type:'delete',ID:scope.row.id}">
...
import { kwhnlzpList, kwhnlzpDel } from "@/api/ynzp/nlzp/kwhnlzp";
